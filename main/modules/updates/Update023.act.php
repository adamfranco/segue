...
// 	public $checkSeparate = true;
	...
	function getDescription () {
		return _("This update will map existing LDAP IDs to CAS IDs if both the LDAP and CAS authentication methods are enabled. It should only be used in transitioning from LDAP to CAS authentication. To use this update, define a global array (named \$update023Types) of authentication types and the attributes to map to the CAS id. As well, you need to specify the CAS type via a global \$update023CasType variable. For example: 
<pre>global \$update023Types, \$update023CasType;

\$update023Types[] = array(
...
\$update023CasType = new Type (\"Authentication\", \"edu.middlebury.harmoni\", \"CAS\");
</pre>
");
	}
	...
	protected function isConfigValid () {
		global $update023Types, $update023CasType;
		$canRun = true;
		$authNMethodManager = Services::getService("AuthNMethodManager");
		
		// Check the source types
		if (is_array($update023Types)) {
			if (count($update023Types)) {
				foreach ($update023Types as $num => $source) {
					// Verify the type
					if (!isset($source['type'])) {
						print "\$update023Types[$num]['type'] is not set. Can't run.<br/>\n";
						$canRun = false;
					} else if ($source['type'] instanceof Type) {
						try {
							$authNMethodManager->getAuthNMethodForType($source['type']);
						} catch (UnknownTypeException $e) {
							print "\$update023Types[$num]['type'] (".$source['type']->asString().") is not an enabled type. Can't run.<br/>\n";
							$canRun = false;
						}
					} else {
						print "\$update023Types[$num]['type'] is not a Type object. Can't run.<br/>\n";
						$canRun = false;
					}
					
					// Verify the source property
					if (!isset($source['cas_id_property']) || !strlen($source['cas_id_property'])) {
						print "\$update023Types[$num]['cas_id_property'] is not set. Can't run.<br/>\n";
						$canRun = false;
					}
				}
			} else {
				print "\$update023Types has no sources. Can't run.<br/>\n";
				$canRun = false;
			}
		} else {
			print "\$update023Types is not set. Can't run.<br/>\n";
			$canRun = false;
		}
		
		// Check the destination type
		if (is_object($update023CasType)) {
			if ($update023CasType instanceof Type) {
				try {
					$authNMethodManager->getAuthNMethodForType($update023CasType);
				} catch (UnknownTypeException $e) {
					print "\$update023CasType (".$update023CasType->asString().") is not an enabled type. Can't run.<br/>\n";
					$canRun = false;
				}
			} else {
				print "\$update023CasType is not a Type object. Can't run.<br/>\n";
				$canRun = false;
			}
		} else {
			print "\$update023CasType is not set. Can't run.<br/>\n";
			$canRun = false;
		}
	...
	function runUpdate () {
		...
		$authNMethodManager = Services::getService("AuthNMethodManager");
		$casAuthNMethod = $authNMethodManager->getAuthNMethodForType($update023CasType);
			...
			foreach ($mappings as $mapping) {
				try {
					...
					if (is_null($casId)) {
						...
						if ($mappingManager->mappingExists($agentId, $casTokens, $update023CasType)) {
							$existing[] = "Mapping already exists for ".$casId." (Agent = ".$mapping->getAgentId()->getIdString().") <br/>\n";
						} else {
							try {
								$mappingManager->createMapping($agentId, $casTokens, $update023CasType);
